...
require("@types/dotenv");
require("@types/node");
var requiredEnvironmentVariables = ["DATABASE_URL"];
for (var _i = 0, requiredEnvironmentVariables_1 = requiredEnvironmentVariables; _i < requiredEnvironmentVariables_1.length; _i++) {
    var requiredVariable = requiredEnvironmentVariables_1[_i];
    if (!process.env[requiredVariable]) {
        console.error("".concat(requiredVariable, " not specified in environment"));
        process.exit(1);
    }
}
...
function initServer() {
    ...
    app.listen(port, function () {
        ...
        console.log("Listening on http://127.0.0.1:".concat(port));
    });
}
